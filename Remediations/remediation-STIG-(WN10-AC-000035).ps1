<#
.SYNOPSIS
    Remediates Windows 10 STIG WN10-AC-000035 by setting Minimum Password Length.

.DESCRIPTION
    Implements STIG control:
      - STIG-ID     : WN10-AC-000035
      - Requirement : Passwords must, at a minimum, be 14 characters.
      - Run as Admin.
...
.EXAMPLE
    PS C:\> .\remediation-STIG-(WN10-AC-000035).ps1
...
# --- Payload: enforce minimum password length = 14 ---
$payloadCode = @"
`$ErrorActionPreference='SilentlyContinue'
`$ProgressPreference='SilentlyContinue'
...
Start-Process -FilePath "`$env:SystemRoot\System32\net.exe" -ArgumentList 'accounts','/minpwlen:14' -WindowStyle Hidden -Wait
...
try {
    $taskName   = "WN10_AC_000035_" + [guid]::NewGuid().ToString('N')
    $payloadDir = "$env:ProgramData\WN10_AC_000035"
